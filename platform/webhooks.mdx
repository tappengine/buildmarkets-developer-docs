...
| `events` | array of strings | ✅ | Event types to subscribe to. Pass an empty array `[]` to receive all events. Must be one of the [valid event types](#valid-event-types) below |

#### Valid event types

Any value in `events` must be one of the following. An unrecognised event name is rejected with `400`.
...
For the full payload each event delivers, see the [Webhook Events Reference](/guides/operations/webhook-setup).
...
| `events` | array of strings | Updated list of subscribed event types. Replaces the existing list. Same [valid event types](#valid-event-types) as on create |
