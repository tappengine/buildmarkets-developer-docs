...
| `last_delivery_at` | string (ISO 8601) \| null | When this endpoint last received a successful delivery. `null` until the first one |
| `last_failure_at` | string (ISO 8601) \| null | When this endpoint last failed a delivery. `null` if it never has |
| `failure_count_24h` | integer | Failed delivery attempts in the last 24 hours |
| `created_at` | string (ISO 8601) | Registration timestamp |
| `updated_at` | string (ISO 8601) | When the webhook was last modified |
  ...
  "data": [
    {
      ...
      "status": "active",
      "last_delivery_at": "2026-03-24T14:32:04Z",
      "last_failure_at": null,
      "failure_count_24h": 0,
      "created_at": "2026-03-24T10:00:00Z",
      "updated_at": "2026-03-24T10:00:00Z"
...
List responses are wrapped in a `data` array with a `pagination` object — they are not a bare array. Pass `limit` and `cursor` query parameters to page through results.
...
```bash
curl https://api.tappengine.com/v1/webhooks/a1b2c3d4-e5f6-7a8b-9c0d-e1f2a3b4c5d6 \
  -H "X-API-Key: $BUILDMARKETS_API_KEY" -H "X-API-Secret: $BUILDMARKETS_API_SECRET"
```
...
```bash
curl -X PATCH https://api.tappengine.com/v1/webhooks/a1b2c3d4-e5f6-7a8b-9c0d-e1f2a3b4c5d6 \
  -H "X-API-Key: $BUILDMARKETS_API_KEY" -H "X-API-Secret: $BUILDMARKETS_API_SECRET" \
  -H "Content-Type: application/json" \
  -d '{"status": "disabled"}'
```
...
```bash
curl -X POST https://api.tappengine.com/v1/webhooks/a1b2c3d4-e5f6-7a8b-9c0d-e1f2a3b4c5d6/test \
  -H "X-API-Key: $BUILDMARKETS_API_KEY" -H "X-API-Secret: $BUILDMARKETS_API_SECRET"
```

Buildmarkets sends a test payload with `"event_type": "system.maintenance_scheduled"` to the registered URL, carrying a `data` object with `message`, `scheduled_at`, and `estimated_duration_minutes`. It is signed with the same HMAC scheme as a real event, so it exercises your signature verification. Your endpoint should respond with a `2xx` status. Check the [delivery history](/guides/operations/webhook-setup) to see whether it was received successfully.
...
```bash
curl -X DELETE https://api.tappengine.com/v1/webhooks/a1b2c3d4-e5f6-7a8b-9c0d-e1f2a3b4c5d6 \
  -H "X-API-Key: $BUILDMARKETS_API_KEY" -H "X-API-Secret: $BUILDMARKETS_API_SECRET"
```
