...
description: "Reference for Buildmarkets HTTP status codes, structured JSON error bodies, common error codes, rate limits, and retry strategies."
---
...
```json
{
  "error": {
    ...
    "details": [
      { "field": "identity.tax_id", "reason": "USA_SSN must be 9 digits." }
    ],
...
| `details` | (When applicable) Array of `{ field, reason }` objects describing each failed field |
...
| Error code | HTTP | Description |
| --- | --- | --- |
| `VALIDATION_ERROR` | 400 | One or more fields failed validation. Check the `details` array. |
...
Buildmarkets enforces rate limits per API key, based on your partner tier. Two limits apply simultaneously — a per-second burst limit and a per-day total:
...
Every response includes `X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`, and `X-RateLimit-Tier` headers so you can track your usage.
