...
2. **The MCP URL**: `https://dev-mcp-api.tappengine.com/mcp`
...
A `partner` key is issued to you by TAPP Engine. An **account token** (`tappacct_…`) is one you mint yourself against an already-`ACTIVE` account, using your partner key — see [Account-scoped tokens](/platform/authentication#account-scoped-tokens) for how to issue, list, and revoke them.
...
```json
{
  "mcpServers": {
    "tapp": {
      "type": "http",
      "url": "https://dev-mcp-api.tappengine.com/mcp",
      "headers": {
        "X-API-Key": "tappacct_…",
        "X-API-Secret": "sk_…",
        "X-User-Type": "account"
...
[mcp_servers.tapp]
command = "npx"
args = ["-y", "mcp-remote", "https://dev-mcp-api.tappengine.com/mcp",
        "--header", "X-API-Key: tappacct_…",
        "--header", "X-API-Secret: sk_…",
        "--header", "X-User-Type: account"]
```
...
```
https://dev-mcp-api.tappengine.com/mcp?api_key=tappacct_…&api_secret=sk_…&user_type=account
```
