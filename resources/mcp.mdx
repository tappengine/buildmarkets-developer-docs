...
A `partner` key is issued by TAPP Engine. An `account` token (`tappacct_…`) is one you mint yourself for a single account with `POST /v1/accounts/{accountId}/keys` — see [Account-scoped tokens](/platform/authentication#account-scoped-tokens).
