...
A `partner` key is issued to you by TAPP Engine. An **account token** (`tappacct_…`) is one you mint yourself, against an account that is already `ACTIVE`, using your partner key:
...
The response returns `key` and `secret` — the **secret is shown exactly once**. Use them as `X-API-Key` and `X-API-Secret` below. The token can only ever act on the one account it was issued for. See [Account-scoped tokens](/platform/authentication#account-scoped-tokens) for the full reference, including listing and revoking.
