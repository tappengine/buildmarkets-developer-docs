...
**Multiple accounts per user:** a single user can hold several accounts. Keep the `user_id` from the account response — once the user's KYC is approved, open additional accounts (for example an `ira_roth` alongside their `individual` account) via `POST /v1/users/{userId}/accounts` with just an `account_type`. Document verification is skipped because the existing approval carries over.
