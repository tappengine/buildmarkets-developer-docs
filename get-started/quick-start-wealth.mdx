---
title: "Quick Start: Wealth Management"
sidebarTitle: "Wealth Management"
description: "Common Buildmarkets integration patterns for embedded brokerage, robo-advisors, challenger banks, and employee benefits."
---
...
**What you'll build:** (1) Collect KYC data and open a brokerage account — `POST /v1/accounts`; (2) Link the user's external bank account via ACH — `POST /v1/accounts/{accountId}/ach-relationships`; (3) Initiate an initial cash deposit — `POST /v1/accounts/{accountId}/funding/deposits`; (4) Display a stock or ETF search using reference data — `POST /v1/marketdata/reference/symbol-details`; (5) Place a market or limit order — `POST /v1/accounts/{accountId}/orders`; (6) Display the portfolio — balances and open positions — `GET /v1/accounts/{accountId}/balances` \+ `/positions`.

**Key account fields:** `account_type: "individual"`, `investment_profile`, `agreements`, `documents` (government ID images for identity verification). **Key order fields:** `symbol`, `asset_class` (`equity` or `etf`), `order_type`, `side`, `quantity` or `notional`, `client_order_id`.

**Multiple accounts per user:** a single user can hold several accounts. Keep the `user_id` from the account response — once the user's KYC is approved, open additional accounts (for example an `ira_roth` alongside their `individual` account) via `POST /v1/users/{userId}/accounts` with just an `account_type` (plus `joint_owner` for a joint account). Document verification is skipped because the existing approval carries over.

## 2. Robo-advisor / automated investing
...
**Key account fields:** `investment_profile.risk_tolerance` (`conservative`, `moderate`, `aggressive`); `investment_profile.investment_objective` (`growth`, `income`, `speculation`, `preservation`); `investment_profile.time_horizon` (`short`, `medium`, `long`); `dividend_reinvestment` (auto-reinvests dividends into the same security). **Key order fields:** Use `notional` (dollar amount) instead of `quantity` (share count) for dollar-cost averaging. Supports `gtc` (good-till-cancelled) time-in-force for limit-based allocation strategies.

## 3. Challenger bank with investing
...
| Deposit / withdrawal | `pending` → `sent` | `completed` |
