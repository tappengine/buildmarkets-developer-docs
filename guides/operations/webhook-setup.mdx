...
```json
{
  "id": "3f8c1d92-5b7a-4e21-9c6d-0a1b2c3d4e5f",
  "event_id": "9f1c2b7e-4d3a-4c8b-9e6f-1a2b3c4d5e6f",
  "event_type": "order.filled",
  "api_version": "v1",
  "created_at": "2026-03-24T14:32:00Z",
  "environment": "sandbox",
  "partner_id": "b3f2c9a1-7d4e-4f8a-9c1b-2e3d4f5a6b7c",
  "data": {
...
| --- | --- | --- |
| `id` | string (UUID) | Identifier for **this delivery attempt**. A retry of the same event carries a different `id` |
| `event_id` | string (UUID) | Identifier for **the event itself**. Stays the same across every retry — use this for deduplication |
| `event_type` | string | The event type name (see tables below) |
| `api_version` | string | Payload schema version. Currently always `v1` |
| `created_at` | string (ISO 8601) | Timestamp when the event was generated |
| `environment` | string | `live` or `sandbox` — which environment produced the event |
| `partner_id` | string (UUID) | The partner the event belongs to |
| `data` | object | Event-specific payload — fields vary by `event_type` |

<Warning>
  `id` and `event_id` are not interchangeable. `id` is regenerated on every delivery attempt, so keying idempotency off it will not catch retries. Always deduplicate on `event_id`.
</Warning>

The per-event examples below show `id`, `event_type`, `created_at`, and `data` only. Every real delivery also carries `event_id`, `api_version`, `environment`, and `partner_id` as shown above.
...
```json
{
  "id": "0f42bc00-5172-44af-978c-9db1e0f56a69",
...
```json
{
  "id": "d2a3535a-8054-468d-915c-f59cfa99a9c0",
...
```json
{
  "id": "58f86c6e-06e7-4491-945a-d514d3a5d07c",
...
```json
{
  "id": "c3d8eee0-9562-453c-9b5d-75a3062685f9",
...
```json
{
  "id": "955bac5c-0e7e-4e11-9f45-1173737e010a",
...
```json
{
  "id": "0351823e-7c43-428c-92e6-77b89aa9442e",
...
```json
{
  "id": "189ac175-9676-4d1b-9e92-47308fc8520c",
...
```json
{
  "id": "4c32b1d4-933f-43fa-9932-e877c5adc242",
...
```json
{
  "id": "958e6be0-f4a0-4949-96c8-f119f9f82206",
...
```json
{
  "id": "ffadb37e-c625-419f-9a59-6e955371aea1",
...
```json
{
  "id": "a785d9cb-b3c6-433d-920a-f92ae629b7b9",
  "event_type": "order.filled",
...
```json
{
  "id": "5a646199-0e8f-466b-94af-75decf026c8d",
...
```json
{
  "id": "3d080358-c337-4467-9ee6-4738a5afce4e",
...
In rare cases (network issues, retries), your endpoint may receive the same event more than once. Use the `event_id` field as an **idempotency key** — store processed event IDs and skip processing if the ID has already been handled.

Do **not** use `id` for this. It identifies a single delivery attempt and is regenerated on every retry, so a duplicate delivery would arrive with an `id` you have never seen and be processed twice.
...
Buildmarkets guarantees **at-least-once delivery** — every event will be delivered at least once, but rare network conditions can result in duplicate deliveries. Use the `event_id` field from the payload envelope as an idempotency key to detect and safely ignore duplicates — it is stable across retries, whereas `id` changes on every attempt.
...
[
  {
    "id": "b8034f8d-648d-413d-9c20-4363fb9e51ba",
    "event_id": "a785d9cb-b3c6-433d-920a-f92ae629b7b9",
    "event_type": "order.filled",
    "status": "succeeded",
    "http_status": 200,
  ...
  {
    "id": "2d58d1de-a96c-4b6a-97e1-2185f348705f",
    "event_id": "ffadb37e-c625-419f-9a59-6e955371aea1",
    "event_type": "order.submitted",
    "status": "failed",
    "http_status": 503,
  ...
  {
    "id": "d43271a6-daf1-4ba9-9eb9-0a241a5735a9",
    "event_id": "ffadb37e-c625-419f-9a59-6e955371aea1",
    "event_type": "order.submitted",
    "status": "succeeded",
    "http_status": 200,
